...
use anyhow::Result;
use chrono::Duration;
use mpd_client::{commands::{self, responses::PlayState}, Client, Subsystem, state_changes::StateChanges};
...
impl MpdND {
    ...
    async fn notify_song(&self) -> Result<()> {
        ...
        if let Some(song_in_queue) = current {
            ...
            notification.appname(APPNAME).summary(&summary).body(&body).timeout(3000);
